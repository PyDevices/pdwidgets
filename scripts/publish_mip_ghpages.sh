...
PDWIDGETS_DIR="${PDWIDGETS_DIR:-$ROOT}"
LIB_DIR="${MIP_DIR:-${MICROPYTHON_LIB_DIR:-$PDWIDGETS_DIR/../mip}}"
...
python3 "$PDWIDGETS_DIR/scripts/build.py" \
    --lib-dir "$LIB_DIR" \
    --micropython "$MPY_DIR" \
    --mpy-cross "$MPY_CROSS" \
    --output "$INDEX_OUT"
...
git commit -m "pdwidgets: Update MIP index from PyDevices/pdwidgets ${SHA}."
