...
(``WIDTH`` / ``HEIGHT`` / ``FORMAT`` / ``BITMAP = bytearray(...)``).
No binary ``.pbm``/``.bmp`` are shipped.
...
Regenerate (from repo root)::
    ...
    # convert → .py (bytearray BITMAP) and remove binaries
    .venv/bin/python scripts/assets_icons_to_py.py --delete-binaries
    # or re-export existing modules after a format change:
